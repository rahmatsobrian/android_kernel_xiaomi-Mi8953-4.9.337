			...
			if (qos_parms->settings) {
				...
				if (rc < 0) {
					...
					for (i = 0; i < qos_entries; i++)
						legacy_n_msm_camera_io_w(qos_settings[i],
							vfebase + qos_regs[i]);
				}
			}
		}
			...
			if (ds_parms->settings) {
				...
				if (rc < 0) {
					...
					for (i = 0; i < ds_entries; i++)
						legacy_n_msm_camera_io_w(ds_settings[i],
							vfebase + ds_regs[i]);
				}
			}
		}
...
static int msm_vfe32_init_hardware(struct vfe_device *vfe_dev)
{
	...
	rc = legacy_n_msm_isp_init_bandwidth_mgr(ISP_VFE0 + vfe_dev->pdev->id);
	if (rc < 0) {
	...
	rc = legacy_n_msm_cam_clk_enable(&vfe_dev->pdev->dev, msm_vfe32_1_clk_info,
		 vfe_dev->vfe_clk, ARRAY_SIZE(msm_vfe32_1_clk_info), 1);
	if (rc < 0) {
		rc = legacy_n_msm_cam_clk_enable(&vfe_dev->pdev->dev,
	...
	rc = request_irq(vfe_dev->vfe_irq->start, legacy_n_msm_isp_process_irq,
...
vfe_remap_failed:
	if (vfe_dev->vfe_clk_idx == 1)
		legacy_n_msm_cam_clk_enable(&vfe_dev->pdev->dev,
...
fs_failed:
	legacy_n_msm_isp_deinit_bandwidth_mgr(ISP_VFE0 + vfe_dev->pdev->id);
...
static void msm_vfe32_init_hardware_reg(struct vfe_device *vfe_dev)
{
	...
	/* CGC_OVERRIDE */
	legacy_n_msm_camera_io_w(0x07FFFFFF, vfe_dev->vfe_base + 0xC);
	/* BUS_CFG */
	legacy_n_msm_camera_io_w(0x00000009, vfe_dev->vfe_base + 0x3C);
	legacy_n_msm_camera_io_w(0x01000021, vfe_dev->vfe_base + 0x1C);
	legacy_n_msm_camera_io_w_mb(0x1CFFFFFF, vfe_dev->vfe_base + 0x20);
	legacy_n_msm_camera_io_w(0xFFFFFFFF, vfe_dev->vfe_base + 0x24);
	legacy_n_msm_camera_io_w_mb(0x1FFFFFFF, vfe_dev->vfe_base + 0x28);
	legacy_n_msm_camera_io_w(0x0, vfe_dev->vfe_base + 0x6FC);
	legacy_n_msm_camera_io_w(0x10000000, vfe_dev->vfe_base + VFE32_RDI_BASE(1));
	legacy_n_msm_camera_io_w(0x10000000, vfe_dev->vfe_base + VFE32_RDI_BASE(2));
	legacy_n_msm_camera_io_w(0x0, vfe_dev->vfe_base + VFE32_XBAR_BASE(0));
	legacy_n_msm_camera_io_w(0x0, vfe_dev->vfe_base + VFE32_XBAR_BASE(4));

}

static void msm_vfe32_clear_status_reg(struct vfe_device *vfe_dev)
{
	legacy_n_msm_camera_io_w((1 << 23), vfe_dev->vfe_base + 0x1C);
	legacy_n_msm_camera_io_w_mb(0x0, vfe_dev->vfe_base + 0x20);
	legacy_n_msm_camera_io_w(0xFFFFFFFF, vfe_dev->vfe_base + 0x24);
	legacy_n_msm_camera_io_w_mb(0xFFFFFFFF, vfe_dev->vfe_base + 0x28);
	legacy_n_msm_camera_io_w_mb(0x1, vfe_dev->vfe_base + 0x18);
}
...
{
	if (irq_status1 & (1 << 24))
		legacy_n_msm_camera_io_w_mb(0, vfe_dev->vfe_base + 0x1D8);
}
	...
	if (irq_status0 & BIT(0)) {
		...
		if (vfe_dev->axi_data.src_info[VFE_PIX_0].raw_stream_count > 0
			...
			if (vfe_dev->axi_data.stream_update)
				legacy_n_msm_isp_axi_stream_update(vfe_dev,
					(1 << VFE_PIX_0));
			legacy_n_msm_isp_update_framedrop_reg(vfe_dev, (1 << VFE_PIX_0));
		}
	}
}
...
{
	*irq0_mask = legacy_n_msm_camera_io_r(vfe_dev->vfe_base + 0x1C);
	*irq1_mask = legacy_n_msm_camera_io_r(vfe_dev->vfe_base + 0x20);
}
...
static void msm_vfe32_read_irq_status(struct vfe_device *vfe_dev,
	uint32_t *irq_status0, uint32_t *irq_status1)
{
	*irq_status0 = legacy_n_msm_camera_io_r(vfe_dev->vfe_base + 0x2C);
	*irq_status1 = legacy_n_msm_camera_io_r(vfe_dev->vfe_base + 0x30);
	legacy_n_msm_camera_io_w(*irq_status0, vfe_dev->vfe_base + 0x24);
	legacy_n_msm_camera_io_w_mb(*irq_status1, vfe_dev->vfe_base + 0x28);
	legacy_n_msm_camera_io_w_mb(1, vfe_dev->vfe_base + 0x18);

	if (*irq_status1 & BIT(0))
		vfe_dev->error_info.camif_status =
			legacy_n_msm_camera_io_r(vfe_dev->vfe_base + 0x204);

	if (*irq_status1 & BIT(7))
		vfe_dev->error_info.violation_status |=
			legacy_n_msm_camera_io_r(vfe_dev->vfe_base + 0x7B4);
}
	...
	struct msm_isp_timestamp *ts)
{
	...
	if (irq_status0 & BIT(5)) {
		legacy_n_msm_isp_notify(vfe_dev, ISP_EVENT_REG_UPDATE, VFE_PIX_0, ts);
		input_src |= (1 << VFE_PIX_0);
	}
	if (irq_status1 & BIT(26)) {
		legacy_n_msm_isp_notify(vfe_dev, ISP_EVENT_SOF, VFE_RAW_0, ts);
		input_src |= (1 << VFE_RAW_0);
	}
	if (irq_status1 & BIT(27)) {
		legacy_n_msm_isp_notify(vfe_dev, ISP_EVENT_SOF, VFE_RAW_1, ts);
		input_src |= (1 << VFE_RAW_1);
	}
	if (irq_status1 & BIT(28)) {
		legacy_n_msm_isp_notify(vfe_dev, ISP_EVENT_SOF, VFE_RAW_2, ts);
		input_src |= (1 << VFE_RAW_2);
	}

	if (vfe_dev->axi_data.stream_update)
		legacy_n_msm_isp_axi_stream_update(vfe_dev, input_src);
	...
	legacy_n_msm_isp_update_framedrop_reg(vfe_dev, input_src);
...
{
	legacy_n_msm_camera_io_w_mb(input_src, vfe_dev->vfe_base + 0x260);
}
	...
	uint32_t first_start, uint32_t blocking)
{
	...
	if (blocking) {
		...
		legacy_n_msm_camera_io_w_mb(0x3FF, vfe_dev->vfe_base + 0x4);
	...
	struct vfe_device *vfe_dev, uint32_t reload_mask)
{
	if (!vfe_dev->pdev->dev.of_node) {
		/*vfe32 A-family: 8960*/
		legacy_n_msm_camera_io_w_mb(reload_mask, vfe_dev->vfe_base + 0x38);
	} else {
		/*vfe32 B-family: 8610*/
		legacy_n_msm_camera_io_w(0x0, vfe_dev->vfe_base + 0x28);
		legacy_n_msm_camera_io_w(0x1C800000, vfe_dev->vfe_base + 0x20);
		legacy_n_msm_camera_io_w_mb(0x1, vfe_dev->vfe_base + 0x18);
		legacy_n_msm_camera_io_w(0x9AAAAAAA , vfe_dev->vfe_base + 0x600);
		legacy_n_msm_camera_io_w(reload_mask, vfe_dev->vfe_base + 0x38);
	}
}
...
{
	uint32_t val = legacy_n_msm_camera_io_r(
	...
	struct msm_vfe_axi_stream *stream_info)
{
	...
	comp_mask = legacy_n_msm_camera_io_r(vfe_dev->vfe_base + 0x34);
	comp_mask &= ~(0x7F << (comp_mask_index * 8));
	comp_mask |= (axi_data->composite_info[comp_mask_index].
		stream_composite_mask << (comp_mask_index * 8));
	legacy_n_msm_camera_io_w(comp_mask, vfe_dev->vfe_base + 0x34);

	irq_mask = legacy_n_msm_camera_io_r(vfe_dev->vfe_base + 0x1C);
	irq_mask |= BIT(comp_mask_index + 21);
	legacy_n_msm_camera_io_w(irq_mask, vfe_dev->vfe_base + 0x1C);
}
	...
	struct msm_vfe_axi_stream *stream_info)
{
	...
	if (stream_info->stream_src == PIX_ENCODER) {
		legacy_n_msm_camera_io_w(framedrop_period, vfe_dev->vfe_base + 0x504);
		legacy_n_msm_camera_io_w(framedrop_period, vfe_dev->vfe_base + 0x508);
		legacy_n_msm_camera_io_w(framedrop_pattern, vfe_dev->vfe_base + 0x50C);
		legacy_n_msm_camera_io_w(framedrop_pattern, vfe_dev->vfe_base + 0x510);
	} else if (stream_info->stream_src == PIX_VIEWFINDER) {
		legacy_n_msm_camera_io_w(framedrop_period, vfe_dev->vfe_base + 0x514);
		legacy_n_msm_camera_io_w(framedrop_period, vfe_dev->vfe_base + 0x518);
		legacy_n_msm_camera_io_w(framedrop_pattern, vfe_dev->vfe_base + 0x51C);
		legacy_n_msm_camera_io_w(framedrop_pattern, vfe_dev->vfe_base + 0x520);
	}
	legacy_n_msm_camera_io_w_mb(0x1, vfe_dev->vfe_base + 0x260);
}
...
{
	if (stream_info->stream_src == PIX_ENCODER) {
		legacy_n_msm_camera_io_w(0, vfe_dev->vfe_base + 0x50C);
		legacy_n_msm_camera_io_w(0, vfe_dev->vfe_base + 0x510);
	} else if (stream_info->stream_src == PIX_VIEWFINDER) {
		legacy_n_msm_camera_io_w(0, vfe_dev->vfe_base + 0x51C);
		legacy_n_msm_camera_io_w(0, vfe_dev->vfe_base + 0x520);
	}
}
	...
	enum msm_vfe_axi_stream_src stream_src, uint32_t io_format)
{
	...
	io_format_reg = legacy_n_msm_camera_io_r(vfe_dev->vfe_base + 0x6F8);
	switch (stream_src) {
	case PIX_ENCODER:
	case PIX_VIEWFINDER:
	case CAMIF_RAW:
	...
	legacy_n_msm_camera_io_w(io_format_reg, vfe_dev->vfe_base + 0x6F8);
	return 0;
}
	...
	struct msm_vfe_pix_cfg *pix_cfg)
{
	...
	legacy_n_msm_camera_io_w(pix_cfg->input_mux << 16 | pix_cfg->pixel_pattern,
					vfe_dev->vfe_base + 0x14);

	legacy_n_msm_camera_io_w(camif_cfg->lines_per_frame << 16 |
	...
	val = legacy_n_msm_camera_io_r(vfe_dev->vfe_base + 0x6FC);
	val &= 0xFFFFFFFC;
	val |= camif_cfg->camif_input;
	legacy_n_msm_camera_io_w(val, vfe_dev->vfe_base + 0x6FC);
}
	...
	enum msm_isp_camif_update_state update_state)
{
	...
	if (update_state == ENABLE_CAMIF) {
		val = legacy_n_msm_camera_io_r(vfe_dev->vfe_base + 0x1C);
		val |= 0x19;
		legacy_n_msm_camera_io_w_mb(val, vfe_dev->vfe_base + 0x1C);
		legacy_n_msm_camera_io_w_mb(0xA, vfe_dev->vfe_base + 0x200);
		val = legacy_n_msm_camera_io_r(vfe_dev->vfe_base + 0x1E4);
		bus_en =
		...
		legacy_n_msm_camera_io_w(val, vfe_dev->vfe_base + 0x1E4);
		legacy_n_msm_camera_io_w_mb(0x4, vfe_dev->vfe_base + 0x1E0);
		legacy_n_msm_camera_io_w_mb(0x1, vfe_dev->vfe_base + 0x1E0);
		vfe_dev->axi_data.src_info[VFE_PIX_0].active = 1;
	} else if (update_state == DISABLE_CAMIF) {
		legacy_n_msm_camera_io_w_mb(0x0, vfe_dev->vfe_base + 0x1E0);
		vfe_dev->axi_data.src_info[VFE_PIX_0].active = 0;
	} else if (update_state == DISABLE_CAMIF_IMMEDIATELY) {
		legacy_n_msm_camera_io_w_mb(0x6, vfe_dev->vfe_base + 0x1E0);
		vfe_dev->axi_data.src_info[VFE_PIX_0].active = 0;
	}
}
	...
	struct msm_vfe_rdi_cfg *rdi_cfg, enum msm_vfe_input_src input_src)
{
	...
	legacy_n_msm_camera_io_w(rdi_reg_cfg,
		vfe_dev->vfe_base + VFE32_RDI_BASE(0));
	...
	uint8_t plane_idx)
{
	...
	if (!stream_info->frame_based) {
		/*WR_IMAGE_SIZE*/
		val =
			((legacy_n_msm_isp_cal_word_per_line(
			stream_info->output_format,
			stream_info->plane_cfg[plane_idx].
			output_width)+1)/2 - 1) << 16 |
			(stream_info->plane_cfg[plane_idx].
			output_height - 1);
		legacy_n_msm_camera_io_w(val, vfe_dev->vfe_base + wm_base + 0x10);
		...
		legacy_n_msm_camera_io_w(0x2, vfe_dev->vfe_base + wm_base);
		val =
			legacy_n_msm_isp_cal_word_per_line(
			stream_info->output_format,
			stream_info->plane_cfg[plane_idx].
	...
	struct msm_vfe_axi_stream *stream_info, uint8_t plane_idx)
{
	...
	uint32_t wm_base = VFE32_WM_BASE(stream_info->wm[plane_idx]);
	/* FRAME BASED */
	legacy_n_msm_camera_io_w(val, vfe_dev->vfe_base + wm_base);
	/*WR_IMAGE_SIZE*/
	legacy_n_msm_camera_io_w(val, vfe_dev->vfe_base + wm_base + 0x10);
	/*WR_BUFFER_CFG*/
	legacy_n_msm_camera_io_w(val, vfe_dev->vfe_base + wm_base + 0x14);
	...
	struct msm_vfe_axi_stream *stream_info, uint8_t plane_idx)
{
	...
	xbar_reg_cfg = legacy_n_msm_camera_io_r(vfe_dev->vfe_base + VFE32_XBAR_BASE(wm));
	xbar_reg_cfg &= ~(0xFF << VFE32_XBAR_SHIFT(wm));
	xbar_reg_cfg |= (xbar_cfg << VFE32_XBAR_SHIFT(wm));
	legacy_n_msm_camera_io_w(xbar_reg_cfg, vfe_dev->vfe_base + VFE32_XBAR_BASE(wm));
	return;
}
	...
	for (i = 0; i < axi_data->hw_info->num_wm; i++) {
		if (axi_data->free_wm[i]) {
			...
			wm_ub_size = axi_data->hw_info->min_wm_ub +
				(uint32_t)delta;
			legacy_n_msm_camera_io_w(ub_offset << 16 |
		...
		uint8_t wm_idx, uint32_t pingpong_status, dma_addr_t paddr)
{
	...
	legacy_n_msm_camera_io_w(paddr32, vfe_dev->vfe_base +
...
static int msm_vfe32_axi_halt(struct vfe_device *vfe_dev, uint32_t blocking)
{
	...
	legacy_n_msm_camera_io_w(0x01800000, vfe_dev->vfe_base + 0x20);
	/*Clear IRQ Status */
	legacy_n_msm_camera_io_w(0xFE7FFFFF, vfe_dev->vfe_base + 0x28);
	legacy_n_msm_camera_io_w_mb(0x1, vfe_dev->vfe_base + 0x1D8);
	while (axi_busy_flag) {
		if (legacy_n_msm_camera_io_r(
	...
	uint32_t blocking, uint32_t enable_camif)
{
	...
	legacy_n_msm_camera_io_w_mb(0xA, vfe_dev->vfe_base + 0x200);
	/* Start AXI */
	legacy_n_msm_camera_io_w(0x0, vfe_dev->vfe_base + 0x1D8);
...
{
	return legacy_n_msm_camera_io_r(vfe_dev->vfe_base + 0x180);
}
...
static void msm_vfe32_stats_cfg_ub(struct vfe_device *vfe_dev)
{
	...
	for (i = 0; i < VFE32_NUM_STATS_TYPE; i++) {
		ub_offset -= ub_size[i];
		legacy_n_msm_camera_io_w(ub_offset << 16 | (ub_size[i] - 1),
	...
	uint32_t stats_mask, uint8_t enable)
{
	...
	module_cfg = legacy_n_msm_camera_io_r(vfe_dev->vfe_base + 0x10);
	if (enable)
		module_cfg |= module_cfg_mask;
	else
		module_cfg &= ~module_cfg_mask;
	legacy_n_msm_camera_io_w(module_cfg, vfe_dev->vfe_base + 0x10);
}
...
{
	legacy_n_msm_camera_io_w(vfe_dev->error_info.overflow_recover_irq_mask0,
		vfe_dev->vfe_base + 0x1C);
	legacy_n_msm_camera_io_w(vfe_dev->error_info.overflow_recover_irq_mask1,
