...
struct msm_isp_bufq *legacy_n_msm_isp_get_bufq(
	...
	uint32_t bufq_handle, uint32_t buf_index)
{
	...
	bufq = legacy_n_msm_isp_get_bufq(buf_mgr, bufq_handle);
	if (!bufq) {
	...
	uint32_t bufq_handle)
{
	struct msm_isp_bufq *bufq =
		legacy_n_msm_isp_get_bufq(buf_mgr, bufq_handle);
	if (!bufq)
		return -EINVAL;
	...
	uint32_t stream_id)
{
	...
	for (i = 0; i < qbuf_buf->num_planes; i++) {
		...
		ret = legacy_n_cam_smmu_get_phy_addr(buf_mgr->iommu_hdl,
					mapped_info->buf_fd,
					CAM_SMMU_MAP_RW,
					&(mapped_info->paddr),
					&(mapped_info->len));
	...
	uint32_t stream_id)
{
	...
	bufq = legacy_n_msm_isp_get_bufq(buf_mgr, buf_info->bufq_handle);
	if (!bufq) {
	...
	for (i = 0; i < buf_info->num_planes; i++) {
		...
		legacy_n_cam_smmu_put_phy_addr(buf_mgr->iommu_hdl, mapped_info->buf_fd);
	}
	return;
}
	...
	struct msm_isp_buffer_mapped_info *mapped_info, uint32_t fd)
{
	...
	return rc;
smmu_map_error:
	legacy_n_cam_smmu_put_phy_addr(buf_mgr->iommu_hdl,
			fd);
	return rc;
}
	...
	uint32_t buf_handle)
{
	...
	struct msm_isp_bufq *bufq = NULL;
	struct msm_isp_buffer *buf_info = NULL;
	bufq = legacy_n_msm_isp_get_bufq(buf_mgr, buf_handle);
	if (!bufq) {
		pr_err("%s: Invalid bufq\n", __func__);
		return rc;
	}
	...
	struct msm_isp_qbuf_info *info)
{
	...
	bufq = legacy_n_msm_isp_get_bufq(buf_mgr, info->handle);
	if (!bufq) {
	...
	struct msm_isp_buf_request *buf_request)
{
	...
	bufq = legacy_n_msm_isp_get_bufq(buf_mgr, buf_request->handle);
	if (!bufq) {
...
static int msm_isp_buf_put_scratch(struct msm_isp_buf_mgr *buf_mgr)
{
	...
	rc = legacy_n_cam_smmu_put_phy_addr_scratch(buf_mgr->iommu_hdl,
...
static int msm_isp_buf_get_scratch(struct msm_isp_buf_mgr *buf_mgr)
{
	...
	rc = legacy_n_cam_smmu_get_phy_addr_scratch(
				buf_mgr->iommu_hdl,
				CAM_SMMU_MAP_RW,
				&buf_mgr->scratch_buf_addr,
...
int legacy_n_msm_isp_smmu_attach(struct msm_isp_buf_mgr *buf_mgr,
	void *arg)
{
	...
	if (cmd->iommu_attach_mode == IOMMU_ATTACH) {
		...
		if (buf_mgr->attach_ref_cnt == 0) {
			rc = legacy_n_cam_smmu_ops(buf_mgr->iommu_hdl,
				CAM_SMMU_ATTACH);
			if (rc < 0) {
...
err2:
	if (legacy_n_cam_smmu_ops(buf_mgr->iommu_hdl, CAM_SMMU_DETACH))
	...
	const char *ctx_name)
{
	...
	rc = legacy_n_cam_smmu_get_handle("vfe", &buf_mgr->iommu_hdl);
	if (rc < 0) {
	...
	struct msm_isp_buf_mgr *buf_mgr)
{
	...
	legacy_n_cam_smmu_ops(buf_mgr->iommu_hdl, CAM_SMMU_DETACH);
	legacy_n_cam_smmu_destroy_handle(buf_mgr->iommu_hdl);
...
int legacy_n_msm_isp_proc_buf_cmd(struct msm_isp_buf_mgr *buf_mgr,
...
static struct msm_isp_buf_ops isp_buf_ops = {
	...
	.get_bufq = legacy_n_msm_isp_get_bufq,
	.update_put_buf_cnt = msm_isp_update_put_buf_cnt,
};

int legacy_n_msm_isp_create_isp_buf_mgr(
	struct msm_isp_buf_mgr *buf_mgr,
