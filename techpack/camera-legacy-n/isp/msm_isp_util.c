	...
	uint32_t force_overflow)
{
	...
	trace_legacy_n_msm_cam_isp_overflow(vfe_dev, *irq_status0, *irq_status1);
...
void legacy_n_msm_isp_dump_irq_debug(void)
{
	...
	for (i = 0; i < count; i++) {
		trace_legacy_n_msm_cam_ping_pong_debug_dump(dump_data.arr[index]);
...
void legacy_n_msm_isp_dump_taskelet_debug(void)
{
	...
	for (i = 0; i < count; i++) {
		trace_legacy_n_msm_cam_tasklet_debug_dump(tasklet_data.arr[index]);
		index = (index + 1) % MAX_ISP_PING_PONG_DUMP_SIZE;
	}
}
...
void legacy_n_msm_isp_dump_ping_pong_mismatch(void)
{
	...
	trace_legacy_n_msm_cam_string(" ***** legacy_n_msm_isp_dump_irq_debug ****");
	legacy_n_msm_isp_dump_irq_debug();
	trace_legacy_n_msm_cam_string(" ***** legacy_n_msm_isp_dump_taskelet_debug ****");
