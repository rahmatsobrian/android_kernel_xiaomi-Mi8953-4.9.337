...
int legacy_n_msm_isp_init_bandwidth_mgr(enum msm_isp_hw_client client)
{
	int rc = 0;
...
static inline void legacy_n_msm_isp_get_timestamp(struct msm_isp_timestamp *time_stamp)
{
	struct timespec ts;
...
static int msm_isp_proc_cmd_list_unlocked(struct vfe_device *vfe_dev, void *arg)
{
	...
	while (cmd.next) {
		...
		rc = legacy_n_msm_isp_proc_cmd(vfe_dev, &cfg_cmd);
		if (rc < 0)
			pr_err("%s:%d failed: rc %d", __func__, __LINE__, rc);
	...
	unsigned int cmd, void *arg)
{
	...
	case VIDIOC_MSM_ISP_AXI_RESET:
		mutex_lock(&vfe_dev->core_mutex);
		rc = legacy_n_msm_isp_axi_reset(vfe_dev, arg);
		mutex_unlock(&vfe_dev->core_mutex);
		break;
	case VIDIOC_MSM_ISP_AXI_RESTART:
		mutex_lock(&vfe_dev->core_mutex);
		rc = legacy_n_msm_isp_axi_restart(vfe_dev, arg);
		mutex_unlock(&vfe_dev->core_mutex);
		break;
...
static void legacy_n_msm_isp_process_overflow_irq(
...
irqreturn_t legacy_n_msm_isp_process_irq(int irq_num, void *data)
{
	...
	legacy_n_msm_isp_get_timestamp(&queue_cmd->ts);
