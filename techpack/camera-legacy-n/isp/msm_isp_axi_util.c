	...
	enum msm_vfe_input_src frame_src, struct msm_isp_timestamp *ts)
{
	...
	case ISP_EVENT_SOF:
		...
		ISP_DBG("%s: vfe %d frame_src %d frame id: %u\n", __func__,
			...
			vfe_dev->axi_data.src_info[frame_src].frame_id);
		trace_legacy_n_msm_cam_isp_bufcount("legacy_n_msm_isp_notify:",
		vfe_dev->pdev->id,
	...
	struct timeval *time_stamp, uint32_t frame_id)
{
	...
	trace_legacy_n_msm_cam_isp_bufcount("msm_isp_process_done_buf:",
	...
	uint32_t frame_id, uint32_t buf_index)
{
	...
	trace_legacy_n_msm_cam_isp_bufcount("msm_isp_request_frame:",
