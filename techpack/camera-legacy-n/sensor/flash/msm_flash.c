...
void legacy_n_msm_torch_brightness_set(struct led_classdev *led_cdev,
...
static struct led_classdev msm_torch_led[MAX_LED_TRIGGERS] = {
	{
		.name		= "torch-light0",
		.brightness_set	= legacy_n_msm_torch_brightness_set,
		.brightness	= LED_OFF,
	},
	{
		.name		= "torch-light1",
				...
				void *data)
{
	...
	for (i = 0; i < fctrl->torch_num_sources; i++) {
		if (fctrl->torch_trigger[i]) {
			...
			CDBG("%s:%d legacy_n_msm_torch_brightness_set for torch %d",
				__func__, __LINE__, i);
			legacy_n_msm_torch_brightness_set(&msm_torch_led[i],
				LED_OFF);
	...
	struct msm_flash_cfg_data_t *flash_data)
{
	...
	rc = legacy_n_msm_camera_power_up(&flash_ctrl->power_info,
		flash_ctrl->flash_device_type,
		&flash_ctrl->flash_i2c_client);
	if (rc < 0) {
		pr_err("%s legacy_n_msm_camera_power_up failed %d\n",
			__func__, __LINE__);
		goto msm_flash_i2c_init_fail;
	}
	...
	struct msm_flash_ctrl_t *flash_ctrl)
{
	...
	rc = legacy_n_msm_camera_power_down(&flash_ctrl->power_info,
		flash_ctrl->flash_device_type,
		&flash_ctrl->flash_i2c_client);
	if (rc < 0) {
		pr_err("%s legacy_n_msm_camera_power_down failed %d\n",
			__func__, __LINE__);
		return -EINVAL;
	}
	return 0;
	...
	const struct i2c_device_id *id)
{
	...
	if (client == NULL) {
		pr_err("legacy_n_msm_flash_i2c_probe: client is null\n");
		return -EINVAL;
	}
	...
	legacy_n_msm_sd_register(&flash_ctrl->msm_sd);
...
static int32_t msm_flash_platform_probe(struct platform_device *pdev)
{
	...
	CDBG("%s:%d flash sd name = %s", __func__, __LINE__,
		flash_ctrl->msm_sd.sd.entity.name);
	legacy_n_msm_cam_copy_v4l2_subdev_fops(&msm_flash_v4l2_subdev_fops);
#ifdef CONFIG_COMPAT
	msm_flash_v4l2_subdev_fops.compat_ioctl32 =
		msm_flash_subdev_fops_ioctl;
#endif
