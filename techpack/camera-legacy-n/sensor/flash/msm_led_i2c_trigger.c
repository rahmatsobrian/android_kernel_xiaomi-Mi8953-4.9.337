...
static void *g_fctrl;
int32_t legacy_n_msm_led_i2c_trigger_get_subdev_id(struct msm_led_flash_ctrl_t *fctrl,
	void *arg)
{
...
int32_t legacy_n_msm_led_i2c_trigger_config(struct msm_led_flash_ctrl_t *fctrl,
	void *data)
{
	int rc = 0;
	int i = 0;
...
int legacy_n_msm_flash_led_init(struct msm_led_flash_ctrl_t *fctrl)
{
	int rc = 0;
...
int legacy_n_msm_flash_led_release(struct msm_led_flash_ctrl_t *fctrl)
{
	int rc = 0, ret = 0;
	struct msm_camera_sensor_board_info *flashdata = NULL;
	struct msm_camera_power_ctrl_t *power_info = NULL;
...
int legacy_n_msm_flash_led_off(struct msm_led_flash_ctrl_t *fctrl)
{
	int rc = 0;
	struct msm_camera_sensor_board_info *flashdata = NULL;
	struct msm_camera_power_ctrl_t *power_info = NULL;
...
int legacy_n_msm_flash_led_low(struct msm_led_flash_ctrl_t *fctrl)
{
	int rc = 0;
	struct msm_camera_sensor_board_info *flashdata = NULL;
	struct msm_camera_power_ctrl_t *power_info = NULL;
...
int legacy_n_msm_flash_led_high(struct msm_led_flash_ctrl_t *fctrl)
{
	int rc = 0;
	struct msm_camera_sensor_board_info *flashdata = NULL;
	struct msm_camera_power_ctrl_t *power_info = NULL;
...
static int set_led_status(void *data, u64 val)
{
	...
	if (val == 0) {
		...
		rc = legacy_n_msm_flash_led_off(fctrl);
		if (rc < 0) {
			pr_err("%s led_off failed line %d\n",
			__func__, __LINE__);
			return rc;
		}
		rc = legacy_n_msm_flash_led_release(fctrl);
		if (rc < 0) {
		...
		rc = legacy_n_msm_flash_led_init(fctrl);
		if (rc < 0) {
			pr_err("%s led_init failed line %d\n",
			__func__, __LINE__);
			return rc;
		}
		rc = legacy_n_msm_flash_led_low(fctrl);
		if (rc < 0) {
...
int legacy_n_msm_flash_i2c_probe(struct i2c_client *client,
		const struct i2c_device_id *id)
{
	...
	rc = legacy_n_msm_led_i2c_flash_create_v4lsubdev(fctrl);
#ifdef CONFIG_DEBUG_FS
...
int legacy_n_msm_flash_probe(struct platform_device *pdev,
	const void *data)
{
	int rc = 0;
	struct msm_led_flash_ctrl_t *fctrl =
